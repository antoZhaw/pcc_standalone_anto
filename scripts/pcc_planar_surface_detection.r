...
# Only Ground points are considered, since we are interested in water points.
# and because it will speed-up computation (fewer points will be considered).
M <- point_metrics(las, ~is.planar(X,Y,Z), k = 20, filter = ~Classification == LASGROUND)
# Merge output with point cloud
las <- add_attribute(las, FALSE, "planar")
las$planar[M$pointID] <- M$planar
plot(las, color = "planar")
# Set a valid class
las$Classification[M$pointID] <- LASWATER
